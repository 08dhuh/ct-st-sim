...
def signal_array(params_tuple, radius, gridnum=500, theta_range=(0, 2 * np.pi),
       ...
       Args:
           ...
           gridnum (int, optional): _description_. Defaults to 500.
           theta_range (tuple, optional): _description_. Defaults to (0, 2 * np.pi).
           phi_range (tuple, optional): _description_. Defaults to (-7 * np.pi / 90, 7 * np.pi / 90).
